...
@synthesize attack;
@synthesize defense;
@synthesize speAttack;
@synthesize speDefense;
@synthesize speed;
...
-(id)initWithName:(NSString *)pName andNumber:(int)pNum
{
    ...
    if (self)
    {
        [self setName:pName];
        [self setNumber:pNum];
        [self setGen:[self selectGeneration:number]];
        [self setType:@"Undefined"];
        [self setHealthPoint:rand() % 50];
        [self setAttack:rand() % 50];
        [self setDefense:rand() % 50];
        [self setSpeAttack:rand() % 50];
        [self setSpeDefense:rand() % 50];
        [self setSpeed:rand() % 50];
    }
    return self;
}
...
- (int)selectGeneration:(int)numPokemon
{
    ...
    else if (numPokemon >= 152 && numPokemon <= 251)
    {
        generation = 2;
    }
    else if (numPokemon >= 252 && numPokemon <= 386)
    {
        generation = 3;
    }
    else if (numPokemon >= 387 && numPokemon <= 493)
    {
        generation = 4;
    }
    else if (numPokemon >= 494 && numPokemon <= 649)
    {
        generation = 5;
    }
    else if (numPokemon >= 650 && numPokemon <= 719)
    {
        generation = 6;
    }
    return generation;
